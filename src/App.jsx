...
/**
 * Fully fledged Habit Tracker (single-file React app)
 * - Habits CRUD
 * - Daily check-ins (per habit per day)
 * - Targets (days per week) + reminders toggle
 * - Dashboard with day/week/month rollups + charts
 * - Streaks, completion rate, best day, consistency
...
// ---------- Data Model ----------
// Habit:
// { id, name, color, targetPerWeek, active, createdAtISO, notes }
// Completion map:
...
function isCompleted(completions, dateISO, habitId) {
  return !!(completions?.[dateISO]?.[habitId]);
}
...
function rollupDaily({ habits, completions, startISO, endISO }) {
  ...
  const series = days.map((iso) => {
    const completedCount = active.reduce(
      (acc, h) => acc + (isCompleted(completions, iso, h.id) ? 1 : 0),
      0
    );
    return {
      dateISO: iso,
...
function rollupWeekly({ habits, completions, weeksBack = 12, weekStartsOn = 1 }) {
  ...
  for (let i = weeksBack - 1; i >= 0; i--) {
    ...
    days.forEach((dISO) => {
      active.forEach((h) => {
        if (isCompleted(completions, dISO, h.id)) perHabit[h.id] += 1;
      });
    });
...
function computeBestStreak({ habitId, completions }) {
  // scan completions days for habit
  ...
  for (const iso of dates) {
    ...
    if (!prev) {
      ...
      const prevD = parseISODate(prev);
      const nextExpected = toISODate(addDays(prevD, 1));
...
function EmptyState({ title, desc, action }) {
  return (
    <div className="border rounded-2xl p-8 text-center bg-background">
...
function Pill({ children }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs bg-background">
      {children}
    </span>
  );
}

function HeatCell({ filled, onClick, title }) {
  return (
    <button
      type="button"
      title={title}
      onClick={onClick}
      className={
        "h-4 w-4 rounded-sm border transition-transform active:scale-95 " +
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="space-y-1">
        ...
        <div className="text-sm text-muted-foreground">
          Track daily wins and watch your consistency compound.
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Pill>
          ...
          >
            <SelectTrigger className="h-7 w-[110px] rounded-full">
        ...
        <Button variant="outline" className="rounded-2xl" onClick={onExport}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="rounded-2xl">
            ...
            <DropdownMenuItem onClick={onReset} className="text-destructive">
              <RefreshCcw className="h-4 w-4 mr-2" /> Reset all data
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="rounded-2xl">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit habit" : "Add a habit"}</DialogTitle>
          <DialogDescription>
            Keep it clear and measurable. You can always tweak targets later.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2">
            <Label htmlFor="habit-target">Target (days per week)</Label>
            <Input
              id="habit-target"
              type="number"
              min={1}
          ...
          <div className="flex items-center justify-between rounded-2xl border p-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Inactive habits won’t count in dashboards.
              </div>
            </div>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={
                "h-6 w-6 rounded-xl border flex items-center justify-center transition-transform active:scale-95 " +
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground truncate">
                {habit.notes || `Target: ${habit.targetPerWeek} days/week`}
              </div>
            </div>
          </div>
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>
                {weekProgress.done}/{weekProgress.target} ({pctClamped}%)
              </span>
            </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="rounded-2xl">
            <DropdownMenuLabel>Habit</DropdownMenuLabel>
            <DropdownMenuSeparator />
  ...
  if (active.length === 0) {
    return (
      <EmptyState
        ...
        desc="Add a habit, or re-activate one in the Habits tab."
      />
    );
  }
  ...
  const daily = useMemo(
    () =>
      rollupDaily({
        habits,
        completions,
        startISO: dailyWindow.startISO,
        endISO: dailyWindow.endISO,
      }),
    [habits, completions, dailyWindow.startISO, dailyWindow.endISO]
  );

  const weekly = useMemo(
    () => rollupWeekly({ habits, completions, weeksBack: 12, weekStartsOn }),
    [habits, completions, weekStartsOn]
  );

  const monthly = useMemo(
    () => rollupMonthly({ habits, completions, monthsBack: 12 }),
    [habits, completions]
  );

  const bestDay = useMemo(() => {
    if (daily.length === 0) return null;
    return daily.reduce((best, cur) => (cur.rate > best.rate ? cur : best), daily[0]);
  }, [daily]);

  const avgRate30 = useMemo(() => {
    if (daily.length === 0) return 0;
    return Math.round(daily.reduce((a, b) => a + b.rate, 0) / daily.length);
  }, [daily]);

  const thisWeek = weekly[weekly.length - 1];
  const lastWeek = weekly[weekly.length - 2];
  const deltaWeek =
    thisWeek && lastWeek ? Math.round(thisWeek.rate - lastWeek.rate) : 0;

  if (active.length === 0) {
    return (
      <EmptyState
        title="Your dashboard is ready"
        desc="Add at least one active habit to start seeing insights here."
      />
    );
  }
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        <Stat
          ...
          sub={`${todaySummary.rate}% complete`}
        />
        <Stat
          icon={Flame}
          label="Average (30d)"
          value={`${avgRate30}%`}
          sub="Daily completion rate"
        />
        <Stat
          icon={CalendarDays}
          label="This week"
          value={`${thisWeek?.rate ?? 0}%`}
          sub={
            lastWeek
              ? `${deltaWeek >= 0 ? "+" : ""}${deltaWeek}% vs last week`
              : ""
          }
        />
        <Stat
          icon={Flame}
          label="Best day (30d)"
          value={bestDay ? `${bestDay.rate}%` : "—"}
          sub={bestDay ? bestDay.label : ""}
        />
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="rounded-2xl shadow-sm">
          <CardHeader>
            <CardTitle className="text-base">Daily completion (30 days)</CardTitle>
            <CardDescription>
              Completed habits per day out of your active list.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="rounded-2xl shadow-sm">
          <CardHeader>
            <CardTitle className="text-base">Weekly completion rate (12 weeks)</CardTitle>
            <CardDescription>
              Overall rate across all active habits and days.
            </CardDescription>
          </CardHeader>
          <CardContent className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <Card className="rounded-2xl shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">Monthly completion (12 months)</CardTitle>
          <CardDescription>
            Total completions across all active habits.
          </CardDescription>
        </CardHeader>
        <CardContent className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
...
function HabitInsights({ habits, completions }) {
  ...
  const rows = useMemo(() => {
    return active
      .map((h) => {
        const currentStreak = computeStreak({ habitId: h.id, completions });
        const bestStreak = computeBestStreak({ habitId: h.id, completions });
        const rate30 = completionRateForHabit({
          habitId: h.id,
          completions,
          startISO,
          endISO,
  ...
  if (active.length === 0) {
    return (
      <EmptyState
        ...
        desc="Add or activate habits to see streaks and rankings."
      />
    );
  }
  ...
  return (
    <Card className="rounded-2xl shadow-sm">
      <CardHeader>
        <CardTitle className="text-base">Habit insights (last 30 days)</CardTitle>
        <CardDescription>
          Streaks and completion rate per habit.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid gap-3">
          {rows.map((r) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Badge className="rounded-full">30d rate: {r.rate30}%</Badge>
              </div>
            </div>
          ))}
        </div>
...
function HabitMatrix({ habits, completions, weekStartsOn }) {
  ...
  if (active.length === 0) {
    return (
      <EmptyState
        title="Heatmap ready"
        desc="Add at least one active habit to see your consistency grid."
      />
    );
  }
  ...
  for (let d = new Date(start); d <= endW; d = addDays(d, 7)) {
    const week = Array.from({ length: 7 }, (_, i) => toISODate(addDays(d, i)));
    weeks.push(week);
  }

  const totalPossible = active.length * daysBack;
  const totalDone = days.reduce((acc, iso) => {
    return acc + active.reduce((a, h) => a + (isCompleted(completions, iso, h.id) ? 1 : 0), 0);
  }, 0);
  ...
  return (
    <Card className="rounded-2xl shadow-sm">
      <CardHeader>
        <CardTitle className="text-base">Consistency heatmap (28 days)</CardTitle>
        <CardDescription>
          Each square is a day. Filled means at least one habit completed.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="flex flex-col gap-2">
                {week.map((iso) => {
                  ...
                  const anyDone =
                    inRange &&
                    active.some((h) => isCompleted(completions, iso, h.id));
                  const title = `${iso} — ${anyDone ? "Some done" : "None"}`;
                  return (
                    <div key={iso} className={inRange ? "" : "opacity-30"}>
                      <HeatCell filled={anyDone} onClick={() => {}} title={title} />
                    </div>
                  );
                })}
              </div>
        ...
        <div className="text-sm text-muted-foreground">
          Tip: Your goal is consistency, not perfection. Focus on showing up.
        </div>
      </CardContent>
    </Card>
  );
    ...
    <div className="space-y-3">
      <Card className="rounded-2xl shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            {activeCount} active · {habits.length} total
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3">
            {habits.length === 0 ? (
              <EmptyState
                title="No habits yet"
                desc="Add your first habit to begin tracking."
              />
            ) : (
              habits
                .slice()
                ...
                .map((h) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate">
                        Target {clamp(h.targetPerWeek || 5, 1, 7)} days/week
                        {h.notes ? ` · ${h.notes}` : ""}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => onUpdate({ ...h, active: !h.active })}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => onUpdate(h, { openEditor: true })}
                      >
                        <Pencil className="h-4 w-4 mr-2" /> Edit
                      </Button>
                      <Button
...
export default function App() {
  ...
  function upsertHabit(h) {
    setState((prev) => {
      ...
      return {
        ...prev,
        habits: prev.habits.map((x) => (x.id === h.id ? { ...x, ...h } : x)),
      };
    });
  }
  ...
  function toggleForDate(habitId, dateISO) {
    setState((prev) => ({
      ...prev,
      completions: toggleCompletion(prev.completions, dateISO, habitId),
    }));
  }

  function resetAll() {
    const ok = window.confirm(
      "Reset all habits and history? This cannot be undone." 
    );
    if (!ok) return;
  ...
  function importData(raw) {
    ...
    const clean = {
      version: 1,
      settings: {
        weekStartsOn: parsed.settings?.weekStartsOn ?? 1,
      },
      ...
      <div className="mx-auto max-w-6xl p-4 sm:p-6 space-y-5">
        <TopBar
          ...
          setWeekStartsOn={(v) =>
            setState((prev) => ({
              ...prev,
              settings: { ...(prev.settings || {}), weekStartsOn: v },
            }))
          }
        />
        ...
        <Tabs defaultValue="today" className="w-full">
          <TabsList className="rounded-2xl">
            ...
            <TabsTrigger value="habits" className="rounded-2xl">Habits</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="today" className="mt-4 space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Stat
                icon={CheckCircle2}
                label="Completed"
                value={selectedDaySummary.completed}
                sub={`of ${selectedDaySummary.total} habits`}
              />
              <Stat
                icon={Flame}
                label="Completion"
                value={`${selectedDaySummary.rate}%`}
                sub={`${selectedDaySummary.remaining} remaining`}
              />
              <Card className="rounded-2xl shadow-sm">
                <CardContent className="p-5">
                  ...
                  <div className="mt-3 text-xs text-muted-foreground">
                    Add a new habit anytime—your history stays intact.
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <TabsContent value="habits" className="mt-4">
            <HabitsManager
              ...
              onUpdate={(habit, opts) => {
                // If caller wants editor, open it instead of silent update
                if (opts?.openEditor) {
                  setHabitEditing(habit);
                  setHabitDialogOpen(true);
                  return;
